	...
	*/
	...
	MultipartRequest multipartRequest = new MultipartRequest(request , saveDirectory , 1024 * 1024 * 100 , "utf-8" , new DefaultFileRenamePolicy());
		...
		*/
		...
		//String uploarder = request.getParameter("uploarder");
		String uploader = multipartRequest.getParameter("uploarder");
		...
		String element = (String)files.nextElement();   // input type="file" 1개를 반환
		 String getOriginalFileName = multipartRequest.getOriginalFileName(element); 	// 서버에 업로드 '한' 파일명을 반환
		 String getFilesystemName	= multipartRequest.getFilesystemName(element); 		// 서버에 업로드 '된' 파일명을 반환
		 String getContentType 		= multipartRequest.getContentType(element);			//파일의 타빙을 반환
		 long length 				= multipartRequest.getFile(element).length();    	    // 파일의 크기를 반환 (long타입)
		
		 
		 System.out.println("uploader : " + uploader);
		 System.out.println("element : " + element);
		 System.out.println("getOriginalFileName : " + getOriginalFileName);
		 System.out.println("getFilesystemName : " + getFilesystemName);
